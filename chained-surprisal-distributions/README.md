...
### `main.py` — discrete explorer (`/main`)
...
### `fixed_point.py` — discrete fixed-point explorer (`/fixed_point`)

A single P1 node plus its first surprisal distribution S(P1), with the same bin-edge and prior controls as `main.py`. Instead of manually deriving children, it automatically **iterates** the surprisal transform (re-bin → re-derive → re-bin …) up to 1000 times per event batch and reports how many iterations it took to converge (tolerance adjustable via a slider), or that it didn't converge. It also:
...
### `continuous_main.py` — continuous (KDE) explorer (`/continuous_main`)

The same chaining/deriving idea as `main.py`, but each node fits a **continuous density** to its events instead of binning them — via Gaussian KDE, adaptive KDE, a Gaussian mixture model, or a B-spline fit (selectable per node), blended with a Gaussian(μ, σ) prior. Deriving a child computes `S(x) = -log2(density(x) · Δx)` at each event, where Δx is an adjustable "bin width" stand-in. Additional controls include a bandwidth/smoothing slider, number-of-GMM-components slider, an event rug overlay with adjustable opacity, and a "trace new events" mode that highlights where newly added events land. A "Working…" indicator appears during slower recomputations (e.g. propagating settings to many descendants). The four bin-simplex viz panels from `fixed_point.py` have no continuous analogue and are not present here.

### `continuous_fixed_point.py` — continuous fixed-point explorer (`/continuous_fixed_point`)

The continuous analogue of `fixed_point.py`: a P1 node and its S(P1) node, both density-fit rather than binned, iterated to a fixed point the same way. Includes a progression view and an overlay chart of the density's shape across iterations, plus the same rug/trace overlays as `continuous_main.py`.

## Prerequisites

- Python 3.8 or newer
- **matplotlib** is required by `fixed_point.py` (via `viz_simplex3d.py`, for the 3D simplex panel) but is **not listed in `requirements.txt`** — install it separately (`pip install matplotlib`) if you plan to run `fixed_point.py`.
...
pip install matplotlib   # needed by fixed_point.py; not in requirements.txt
```
...
http://localhost:5006/fixed_point
http://localhost:5006/continuous_main
http://localhost:5006/continuous_fixed_point
```
...
When `/main` or `/continuous_main` loads you will see:
...
On `/fixed_point` or `/continuous_fixed_point`, just add events — the app automatically iterates and reports the number of iterations to convergence (or "did not converge") without any manual deriving.

## Running the Tests

`test_app.py` is a `pytest` + Playwright end-to-end suite (it launches its own Bokeh server subprocess and drives it with a real browser). **Note:** as of this writing it still references the app's old filename — it starts `bokeh serve foo.py` and requests `http://localhost:5007/foo` — so it will fail to launch until those references are updated to `main.py` / `/main`. There is no `tests/` directory or Playwright JS spec file in this project despite `playwright.config.js` and `package.json` existing; those currently point at the same stale `foo.py` name.
...
├── main.py                      # Discrete explorer app (/main)
├── fixed_point.py                # Discrete fixed-point app (/fixed_point)
├── continuous_main.py            # Continuous (KDE) explorer app (/continuous_main)
├── continuous_fixed_point.py     # Continuous (KDE) fixed-point app (/continuous_fixed_point)
├── events.py                    # Event-generation helper (distribution families/samplers)
├── bin_selection.py              # Bin-frequency tracker + "lock bins" UI (used by fixed_point.py)
...
├── viz_simplex3d.py              # 3D simplex viz panel (fixed_point.py only; needs matplotlib)
├── viz_radial.py                 # Radial/spoke viz panel (fixed_point.py only)
├── viz_scatter_matrix.py         # Scatter-plot-matrix viz panel (fixed_point.py only)
├── viz_parallel_coords.py        # Parallel-coordinates viz panel (fixed_point.py only)
...
**`ModuleNotFoundError: No module named 'matplotlib'` when loading `fixed_point.py`**
...
Either stop the existing process or pass a different port: `bokeh serve main.py fixed_point.py continuous_main.py continuous_fixed_point.py --port 5007`.
...
The test file hasn't been updated since the app was renamed from `foo.py` to `main.py`. Update the `bokeh serve` command and URL inside `test_app.py` (and `playwright.config.js`, if you use it) to reference `main.py` / `/main` before running.
